...
public class CityProject {
    ...
    public static void main(String[] args) {
        ...
        // instatiate a new scrollable map of the cities and links
         ...
         findShortestPath(cities, cityCount, cityString1, cityString2);
...
static void PrintAdjacencyLists(int cityCount, City[] cities) {
    ...
    ***************************************************************************/ 
    ...
    a stack.
    */
    public static void findShortestPath(City[] cities, int cityCount, String cityStartStr, String cityEndStr) {
        
        ArrayList<City> unvisitedCities = new ArrayList<City>();    //Stores our unvisited cities
        ArrayList<City> visitedCities = new ArrayList<City>();      //Stores our visited cities
        
        //Populating our unvisited cities list
        for(int i=0; i<cityCount; i++) {
        ...
        */
        ...
        while(!unvisitedCities.isEmpty()) {
            if(currentCity.getVisited() == false) {
                ...
                //Initiate adjacency node searching. While the current adjacent node in the 
                ...
                while(currentAdjNode != null) {
                        ...
                        //Current city value plus adjacenct city's distance
                        ...
                        if(cityCost < currentAdjNode.getCity().getBestDistance()) {
                            ...
                            currentAdjNode.getCity().setImmediatePredecessor(currentCity);
                        }
                        
                        //If the newly calculated distance is the lowest we've seen in this
                        //city's adjacency list, set that city as our next current city
                        if(cityCost <= nextCityCounter) {
                            nextCityCounter = cityCost;     //Next city counter holds the vertex at the lowest-cost edge
                            newCurrentCity = currentAdjNode.getCity();  //Variable to hold our next city until we're out of the loop
                        }
                        
                        //Move our pointer to the next adjacent node and loop back until we've reached currentAdjNode = null
                ...
                if(newCurrentCity != null && newCurrentCity.getVisited() == false) {
                    visitedCities.add(currentCity);     //Add current city to our visited cities
                    unvisitedCities.remove(currentCity);    //Remove current city from our unvisited cities
                    currentCity = newCurrentCity;       //Move our current city pointer to the next city
                    
                    nextCityCounter = Integer.MAX_VALUE;    //Reset nextCityCounter
                    cityCounter++;
                }
                
                /*
                Occasionally the algorithm will reach a dead end and the currentCity will
                be visited. When this happens, we reinitiate the adjNode traversal from the
                next shortest distance in our unvisited cities list
                */
                if(currentCity.getVisited() == true) {
                    visitedCities.add(currentCity);
                    unvisitedCities.remove(currentCity);
                    int shortestBestDistance = Integer.MAX_VALUE;
                    City tempNewCurrentCity = new City();
                    //Finding our next closest city
                    for(City c : unvisitedCities) {
                        if(c.getBestDistance() < shortestBestDistance) {
                            shortestBestDistance = c.getBestDistance();
                            tempNewCurrentCity = c;
                        }
                    }
                    //Making sure the next city exists
                    if(tempNewCurrentCity != null) {
                        currentCity = tempNewCurrentCity;
                        cityCounter++;
                    }
                }
        }// end main Djikstra's loop
        
        
        Stack<City> cityStack = new Stack<>();  //Initialize our Stack
        System.out.println("Printing the shortest path from " + cityStart + " to " + cityEnd + " ...");
        printShortestPath(cityStack, cityEnd);  //Present our shortest path to the user
        
    }// end findShortestPath method
    
    /*
    Prints the stack populated by iterating over the immediatePredeccesor property on our cityEnd
    */
    public static void printShortestPath(Stack<City> cityStack, City c) {
            cityStack.push(c);      //Push the final city to our stack
            City tempPredecessor = c.getImmediatePredecessor();     //Set our immediatePredecessor variable
            while(tempPredecessor != null) {        //Recursively explore all predecessors
                cityStack.push(tempPredecessor);    //Push our current predecessor to the stack
                tempPredecessor = tempPredecessor.getImmediatePredecessor();    //Set current predecessor to its next predecessor
            }
            //Pop the whole stack off to display the shortest path
            while(!cityStack.isEmpty()) {
                System.out.println(cityStack.pop());
            }
        
    }// end printShortestPath method
